...
*======================================================================================================================
	...
	* 6. Calculate propensity scores (pscores)
		...
		*	Product of one-minus-mid (lambda)
			...
			by StudentID : replace lambda = lambda[_n-1] * one_minus_mid if (_n > 1) & (NonLottery[_n-1] == 0)
			by StudentID : replace lambda = lambda[_n-1] 				 if (_n > 1) & (NonLottery[_n-1] == 1)
			
			/*
			1.non below bw
			2.non in bw 0.5
			3.non-lottery in bw 0.5 
			4.non-lottery in bw 0.5
			5.lottery
			6.non-lottery
			7.lotter
			*/
