...
{p 4 8}{cmd:rdmd} {hline 2} 2SLS regression analysis for evaluating program effectiveness on applicants through the calculation and control of local propensity scores using central assignment data{p_end}
...
{synopt :{cmd:10}}An applicant is assigned to a school s although the applicant was guaranteed an assignment to a school s' that she prefers to s and was not assigned to s'{p_end}
{synopt :{cmd:11}}An applicant is assigned to school s, even though (1) she prefers school s' to s, (2) her applicant position at s' was better than her position at s, (3) there were still available spots at s', (4) and she is eligible at s'{p_end}
...
{p 4 8}If {cmd:rdmd} is executed without options specified, the user will be prompted to select the type of bandwidth estimation for calculating the local propensity scores. The user has two options: {cmd:IK} or {cmd:CCFT}. The default is {cmd:IK}.{p_end}
